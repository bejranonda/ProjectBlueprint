...
Option cards (`OptionCard.jsx`, `OtherOptionCard.jsx`) are non-native clickable elements made keyboard-accessible: each has `role="radio"` (single) or `role="checkbox"` (multiple), `aria-checked`, an Enter/Space `onKeyDown` handler, a visible `focus-visible` ring, and a `tabIndex` passed down from the parent. The options grid in `QuestionScreen.jsx` wraps them in a matching `radiogroup`/`group` role and implements the WAI-ARIA radio pattern for single-select: a roving tabindex (only the selected/first card is tabbable) plus Arrow-key navigation that moves focus and selection together. Preserve these attributes — and the `tabIndex` prop plumbing — when editing the cards.
